...
// contribution to energy and virial
  ...
  Instant.open("InstantValues_" + state + ".out", ios::app);
  Instant << walker[iv]/double(npart) + vtail << setw(wd) << walker[iw] + rho * temp + ptail * (double)npart / vol << endl;
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Epot.open("output." + state + ".epot.0",ios::app);
    Pres.open("output." + state + ".pres.0",ios::app);
    Gofr.open("output." + state + ".gofr.0",ios::app);
