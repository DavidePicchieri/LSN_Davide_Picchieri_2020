...
{
	return 1.0;
//	double norm = sqrt(2.*sqrt(M_PI)*sigma*(1.+exp(-1.*mu*mu/(sigma*sigma))));
//	return (exp(-(v-mu)*(v-mu)/(2.*sigma*sigma))+exp(-(v+mu)*(v+mu)/(2.*sigma*sigma)))/norm;
}
...
double variationalWaveFunction_second(double v)
{
	return 0;
//	double norm = sqrt(2.*sqrt(M_PI)*sigma*(1.+exp(-1.*mu*mu/(sigma*sigma))));
//	return (exp(-(v-mu)*(v-mu)/(2.*sigma*sigma))*(pow((v-mu)/(sigma*sigma),2)-1.)+exp(-(v+mu)*(v+mu)/(2.*sigma*sigma))*(pow((v+mu)/(sigma*sigma),2)-1.))/norm;
}
